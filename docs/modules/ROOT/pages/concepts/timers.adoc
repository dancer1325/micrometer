...
* Timers
    ** goal
        *** measure short-duration latencies & frequency of such events
    ** ALL implementations of `Timer` -- report -- the
        *** total time
        *** count of events -- as -- separate time series (OTHER time series can also be reported)
    ** values
        *** negative values NOT supported
        *** `Long.MAX_VALUE` nanoseconds (292.3 years)
            **** if it's close to it -> can cause overflow
    ** base unit -- `TimeUnit` --
        *** -- depends on -- metrics backend
        *** required | interacting with `Timer` implementations
            **** == Micrometer publishes your timing | appropriate base unit

* _Examples:_
    ** _Example1:_ graph / shows request latency | typical web server
        *** server -- can be expected to respond to -- MANY requests quickly
            **** timer gets updated many times / 1"
    ** _Example2:_ fluent builder for timers
+
[source,java]
----
Timer timer = Timer
